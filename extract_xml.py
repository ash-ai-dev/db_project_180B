"""
Extract university data from .xml to .csv format
...
def iter_children(elem, wanted_tag: str):
    """iterate child elements by local name (namespace-safe)."""
...
def extract(xml_path: str, out_dir: str):
    ...
    rooms_w = CsvWriter(
        os.path.join(out_dir, "rooms.csv"),
        ["room_id", "capacity", "location_x", "location_y",
         "has_constraints"]
    )
    room_patterns_w = CsvWriter(
        os.path.join(out_dir, "room_sharing_patterns.csv"),
        ["room_id", "unit_slots", "free_for_all_char",
         "not_available_char", "pattern_text"]
    )
    room_depts_w = CsvWriter(
        os.path.join(out_dir, "room_sharing_departments.csv"),
        ["room_id", "digit_char", "department_id"]
    )

    classes_w = CsvWriter(
        os.path.join(out_dir, "classes.csv"),
        ["class_id", "offering_id", "config_id", "subpart_id",
         "committed", "class_limit", "scheduler", "dates_mask"]
    )

    instructors_w = CsvWriter(
        os.path.join(out_dir, "instructors.csv"),
        ["instructor_id"]
    )

    class_instr_w = CsvWriter(
        os.path.join(out_dir, "class_instructors.csv"),
        ["class_id", "instructor_id"]
    )
    class_roomopt_w = CsvWriter(
        os.path.join(out_dir, "class_room_options.csv"),
        ["class_id", "room_id", "pref"]
    )
    class_timeopt_w = CsvWriter(
        os.path.join(out_dir, "class_time_options.csv"),
        ["class_id", "days_mask", "start_slot", "length_slots", "pref"]
    )

    constraints_w = CsvWriter(
        os.path.join(out_dir, "constraints.csv"),
        ["pk", "external_id", "type", "pref_raw", "pref_numeric"]
    )
    constr_classes_w = CsvWriter(
        os.path.join(out_dir, "constraint_classes.csv"),
        ["constraint_pk", "order_index", "class_id"]
    )

    students_w = CsvWriter(
        os.path.join(out_dir, "students.csv"),
        ["student_id"]
    )
    stud_offerings_w = CsvWriter(
        os.path.join(out_dir, "student_offerings.csv"),
        ["student_id", "offering_id", "weight"]
    )
    stud_classes_w = CsvWriter(
        os.path.join(out_dir, "student_classes.csv"),
        ["student_id", "class_id"]
    )
    stud_prohibited_w = CsvWriter(
        os.path.join(out_dir, "student_prohibited_classes.csv"),
        ["student_id", "class_id"]
    )
    ...
    for event, elem in context:
        ...
        # rooms (with optional sharing)
        if tag == "room":
            # IMPORTANT: only process top-level room definitions
            ...
            if sharing is not None:
                ...
                unit_slots = (
                    to_int(pattern_el.attrib.get("unit"))
                    if pattern_el is not None else None
                )
                pattern_text = (
                    (pattern_el.text or "").strip()
                    if pattern_el is not None else None
                )

                ffa_el = next(iter_children(sharing, "freeForAll"), None)
                not_av_el = next(
                    iter_children(sharing, "notAvailable"), None
                )
                free_for_all_char = (
                    ffa_el.attrib.get("value")
                    if ffa_el is not None else None
                )
                not_available_char = (
                    not_av_el.attrib.get("value")
                    if not_av_el is not None else None
                )
                room_patterns_w.writerow([
                    rid, unit_slots, free_for_all_char,
                    not_available_char, pattern_text
                ])
        ...
        # CLASS DEFINITIONS (only real ones under <classes>)
            ...
            # Heuristic: only treat as a real "class definition"
            # if it has class-def attributes
            is_definition = (
            ...
            if not is_definition:
                # This is a reference (likely under <students>
                # or <constraint>) — ignore here.
                # DO NOT clear: parent (<student> / <constraint>)
                # still needs it.
                continue
            ...
            classes_w.writerow([
                cid, offering, config, subpart, committed,
                class_limit, scheduler, dates_mask
            ])
            ...
            for t in iter_children(elem, "time"):
                ...
                class_timeopt_w.writerow([
                    cid, days_mask, start_slot, length_slots, pref
                ])
        ...
        # CONSTRAINTS + class membership
            ...
            next_constraint_pk += 1
            constraints_w.writerow([
                pk, external_id, ctype, pref_raw, pref_numeric
            ])
    ...
    for w in (rooms_w, room_patterns_w, room_depts_w,
              ...
              students_w, stud_offerings_w, stud_classes_w,
              stud_prohibited_w):
        w.close()
...
def main():
    ...
    # Input XML (change the filename below if yours is different)
